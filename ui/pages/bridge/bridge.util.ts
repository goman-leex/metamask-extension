...
// TODO account for blocked tokens, should have a field denoting block status within token
export async function fetchDestTokens(
  chainId: Hex,
): Promise<Record<string, SwapsTokenObject>> {
  // TODO make token api v2 call, include blocked tokens?
  ...
  const tokens = await fetchWithCache({
    ...
    fetchOptions: { method: 'GET', headers: CLIENT_ID_HEADER },
    cacheOptions: { cacheRefreshTime: CACHE_REFRESH_TEN_MINUTES },
    functionName: 'fetchDestTokens',
  });

  const nativeToken = SWAPS_CHAINID_DEFAULT_TOKEN_MAP[chainId] || null;
